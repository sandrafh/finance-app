...
export const ExpensesService = () => {
  ...
  const addExpense = (expense: Partial<Expense>) => {
    try {
      ...
      if (category) {
        db()
          .ref(`users/${userUid}/categories/${categoryUid}/expenses/${expenseUid}`)
          .set({ spent: expense.spent, date: expense.date })
        ...
        const parentCategory = categories.find(
          (category: any) => category.uid === subCategory.parentCategoryUid
        ) as Category

        //Add expense to parent category
        db()
          .ref(`users/${userUid}/categories/${parentCategory.uid}/expenses/${expenseUid}`)
          .set({ spent: expense.spent, date: expense.date })
        const newTotalSpentParentCategory = +parentCategory.totalSpent + (expense.spent as number)
        db()
          .ref(`users/${userUid}/categories/${parentCategory.uid}`)
          .update({ totalSpent: newTotalSpentParentCategory })

        //Add expense to category itself
        db()
          .ref(`users/${userUid}/categories/${parentCategory.uid}/categories/${subCategory.uid}/expenses/${expenseUid}`)
          .set({ spent: expense.spent, date: expense.date })
        const newTotalSpentCategory = +subCategory.totalSpent + (expense.spent as number)
        db()
          .ref(`users/${userUid}/categories/${parentCategory.uid}/categories/${subCategory.uid}`)
          .update({ totalSpent: newTotalSpentCategory })
      }
    } catch (e) {
  ...
  const deleteExpense = (expenseUid: string) => {
    try {
      ...
      if (!category.parentCategoryUid) {
        ...
        db()
          .ref(`users/${userUid}/categories/${parentCategory.uid}/categories/${category.uid}/expenses/${expenseUid}`)
          .remove()
        ...
        db()
          .ref(`users/${userUid}/categories/${parentCategory.uid}/categories/${category.uid}`)
          .update({ totalSpent: newTotalSpentCategory })
      }
    } catch (e) {
