...
export const CategoryService = () => {
          ...
          if (data) {
            ...
            categories.forEach((category) => {
              if (category?.expenses) {
                category.expenses = Object.entries(category.expenses).map(([key, value]: any) => ({
                  ...value,
                  uid: key,
                }))
              }
              if (category?.categories) {
                category.categories = Object.entries(category.categories).map(([key, value]: any) => ({
                  ...value,
                  uid: key,
                }))
  ...
  const updateCategory = (category: Partial<Category>) => {
    try {
      if (category.parentCategoryUid) {
        db()
          .ref(`users/${userUid}/categories/${category.parentCategoryUid}/categories/${category.uid}`)
          .update(category)
        return
      }
